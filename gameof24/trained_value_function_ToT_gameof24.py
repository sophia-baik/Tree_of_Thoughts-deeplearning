...
numbers = util_gameof24.import_data() 
...
def complete_one_problem(quad: list[int], b: int):
    ...
    for i in range(b):
        ...
        try:
            before_equals, after_equals = util_gameof24.parse_math_expression(response)
        except ValueError:
            print("No valid math found, skipping.")
            continue
    ...
    for before, after in valid_responses:
        candidate_numbers = find_remaining_nums(quad, before, after)
        score = evaluate_state(candidate_numbers)
        remaining_numbers.append((score, candidate_numbers, before, after))

    remaining_numbers.sort(reverse=True, key=lambda x: x[0])
    top_remaining = remaining_numbers[:b]
    remaining_numbers = [x[1] for x in top_remaining]
    ...
    for nums in remaining_numbers:
        for _ in range(3):
            ...
            # This is quite hard coded; we'll be in trouble if Chat deviates from instructions
            try:
                child_before, child_after = util_gameof24.parse_math_expression(child_response)
            except ValueError:
                print("No valid math found, skipping.")
                continue
