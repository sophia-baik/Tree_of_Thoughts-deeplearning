...
data_path = "data/24game_problems.csv"
    ...
    def dfs(state, depth, path):
        ...
        for (i, j) in itertools.combinations(range(len(state)), 2):
            ...
            for new_num in candidates:
                ...
                new_state = [state[k] for k in range(
                    len(state)) if k != i and k != j] + [new_num]
...
class ValueNetwork(nn.Module):
    def __init__(self, input_size=37, hidden_sizes=[128, 128, 64], dropout_rates=[0.3, 0.3, 0.2]):
        ...
        self.skip_connection = nn.Linear(
            hidden_sizes[0], hidden_sizes[1]) if hidden_sizes[0] != hidden_sizes[1] else nn.Identity()
